...
class PopperObject {
  ...
  clicker(event, popperTarget, reference) {
    if (
      sidebar && sidebar.classList && sidebar.classList.contains("collapsed") &&
      popperTarget && !popperTarget.contains(event.target) &&
...
const slideUp = (target, duration = ANIMATION_DURATION) => {
  ...
  if (parentElement && parentElement.classList) parentElement.classList.remove("open");
...
const slideDown = (target, duration = ANIMATION_DURATION) => {
  const { parentElement } = target;
  if (parentElement && parentElement.classList) parentElement.classList.add("open");
...
(() => {
  ...
  if (
    !localStorage.getItem("nowalayout") &&
    !localStorage.getItem("nowanavstyles") &&
    !localStorage.getItem("nowaverticalstyles") &&
  ...
  function rtlFn() {
    ...
    //RTL
    if (localStorage.getItem('nowartl')) {
      ...
      switch (verticalStyle) {
        ...
        case "overlay":
          ...
          if (html.getAttribute('data-toggled') === 'icon-overlay-close') {
            ...
            if (window.innerWidth >= 992) {
              if (!localStorage.getItem("nowalayout")) {
...
function checkHoriMenu() {
  ...
  let slideLeft = document.querySelector('.slide-left');
  let slideRight = document.querySelector('.slide-right');
  if (!menuNav || !mainContainer1 || !slideLeft || !slideRight) return;
  let marginLeftValue = Math.ceil(Number(window.getComputedStyle(menuNav).marginLeft.split('px')[0]));
  let marginRightValue = Math.ceil(Number(window.getComputedStyle(menuNav).marginRight.split('px')[0]));
  let check = menuNav.scrollWidth - mainContainer1.offsetWidth;
  // Show/Hide the arrows
  if (menuNav.scrollWidth > mainContainer1.offsetWidth) {
    if (slideRight.classList) slideRight.classList.remove('d-none');
    if (slideLeft.classList) slideLeft.classList.add('d-none');
  }
  else {
    if (slideRight.classList) slideRight.classList.add('d-none');
    if (slideLeft.classList) slideLeft.classList.add('d-none');
    menuNav.style.marginLeft = '0px';
    menuNav.style.marginRight = '0px';
  }
...
['switcher-icon-click', 'switcher-icon-hover'].map((element) => {
